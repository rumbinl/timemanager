...
TM_StorageManager::TM_StorageManager(std::string dbFilePath)
{
    ...
    if((errorCode = sqlite3_exec(this->dbPtr, "CREATE TABLE IF NOT EXISTS TimeSections (Name varchar(255), startDatetime DATETIME, endDatetime DATETIME, headTaskID INTEGER, ID INTEGER PRIMARY KEY AUTOINCREMENT);", NULL, NULL, NULL)) != SQLITE_OK)
...
{
    std::string queryString = "UPDATE TimeSections SET Name='"+taskPtr->getName()+"', startDatetime='"+TM_GetDateTimeString(taskPtr->getStartDate(), taskPtr->getStartTime())+"', endDatetime='"+TM_GetDateTimeString(taskPtr->getEndDate(), taskPtr->getEndTime())+"', headTaskID="+std::to_string(taskPtr->getHeadTaskID())+" WHERE ID="+std::to_string(taskPtr->getDBID())+";";
    int errorCode;
    if((errorCode = sqlite3_exec(this->dbPtr, queryString.c_str(), NULL, NULL, NULL)) != SQLITE_OK)
        std::cout<<sqlite3_errmsg(this->dbPtr)<<std::endl;
}
...
void TM_StorageManager::LoadTasks(TM_TaskManager* taskManPtr)
{
    ...
    if((errorCode = sqlite3_exec(this->dbPtr, queryString.c_str(), [](void* x, int columnCount, char** columnData, char** columnNames) -> int {
        ...
        taskManPtr->addTask(new TM_Task(columnData[NAME_COLUMN_IDX], startDate, endDate, startTime, endTime, std::atoi(columnData[ID_COLUMN_IDX]), std::atoi(columnData[HEADID_COLUMN_IDX])));
